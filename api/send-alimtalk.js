...
export default async function handler(req, res) {
  // CORS 규격 준수: 모든 Origin 허용 (Access-Control-Allow-Origin: *)
  ...
  try {
    ...
    const { name, phone, programTitle, duration, date, time, note } = body;
    const cleanedPhone = (phone || '').replace(/[^0-9]/g, '');

    if (!cleanedPhone) {
      return res.status(200).json({ success: false, error: '휴대폰 번호가 올바르지 않습니다.' });
    }
    ...
    if (!solapiRes.ok) {
      return res.status(200).json({
        success: false,
        error: solapiData.errorMessage || solapiData.message || `Solapi 오류 (상태: ${solapiRes.status})`
      });
    }
    ...
    return res.status(200).json({ success: false, error: error.message || '알림톡 발송 중 서버 오류가 발생했습니다.' });
  }
}
